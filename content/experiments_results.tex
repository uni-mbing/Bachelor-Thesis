...
We choose a timestep of $dt=0.01$ in the simulations, corresponding to $400s$ and let the them run for a dimensionless time of $t=8$ corresponding to $320000s=88$ $days$. Looking at the results below, it was found that for every unit of time in Anderson et al.'s experiments, $0.4$ units of time have passed here.

Another challenge is the diffusion and haptotaxis coefficients. Since they depend on the dimension we are in, we first have to find our own estimates as a baseline value. They will be used as baseline values as a foundation for the Parameter Analysis. In the Parameter Analysis one or more parameters at a time are varied, to get an overview of their effects. 
...
Before starting with the experiments there is need to discuss the system's initial conditions. They are stated for three dimensions, but for two or one dimension the $y$- or $z$-terms need to be left out.
